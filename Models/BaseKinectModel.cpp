...
{
	BaseKinectModel *model = new BaseKinectModel();
	...
	model->pointCloud = std::vector<cv::Point3f>(sizeOfTheFrame);
	...
	for (int i = 0; i < rawDepthData.size() ; i++)
	{
		...
		{//if raw depth data is valid. the very first generetion kinect currently can detect distance between 1000 mm to 3500 mm 
			...
			cv::Point3f Point3D = DimensionConverter::DepthTo3D(x, y, rawDepthData[i]);
